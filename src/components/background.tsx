...
export const Background = ({ src, alt, children, className, parentClassName, imgClassName }: BackgroundProps) => {
  return (
    <figure className={`relative text-light shadow-lg overflow-hidden group ${parentClassName ?? ""}`}>
      <Image src={src} alt={alt} placeholder={`data:image/svg+xml;base64,${toBase64(shimmer(400, 400))}`} fill quality={100} priority className={`absolute inset-0 w-full h-full ${imgClassName}`} />
