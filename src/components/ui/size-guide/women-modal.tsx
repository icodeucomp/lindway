...
export const WomenModal = ({ isVisible, onClose }: { isVisible: boolean; onClose: () => void }) => {
  ...
  return (
    <Modal isVisible={isVisible} onClose={onClose}>
      <h4 className="mb-4 text-center heading">Women&apos;s Size Guide</h4>
      <div className="max-w-2xl py-4 mx-auto bg-light">
        <div className="flex justify-between mb-6 border-b border-gray">
          <button
            onClick={() => setActiveTab("kebaya")}
            className={`px-4 py-2 flex-1 font-medium text-sm border-b-2 transition-colors ${activeTab === "kebaya" ? "border-gray text-gray" : "border-transparent text-dark"}`}
          >
            For Kebaya & Everyday Wear
          </button>
          <button
            onClick={() => setActiveTab("batik")}
            className={`px-4 py-2 flex-1 font-medium text-sm border-b-2 transition-colors ${activeTab === "batik" ? "border-gray text-gray" : "border-transparent text-dark"}`}
          >
            Batik Skirts
          </button>
        </div>
