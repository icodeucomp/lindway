...
export const ContactUs = () => {
  return (
    <Background src="/images/contact-us-header-background.webp" alt="hero background" className="flex flex-col items-center min-h-600 bg-dark/30">
      <Header />
      <Motion tag="div" initialX={0} animateX={0} duration={0.3} className="w-full space-y-8 pt-20 pb-8 md:pb-0 md:pt-24 max-w-screen-xl px-4 sm:px-8">
          ...
          <Motion tag="div" initialY={50} animateY={0} duration={0.9} delay={0.3} className="flex justify-between flex-wrap w-full gap-4">
            {contactUs.map((item, index) => (
              <div key={index} className="flex gap-2 items-center sm:whitespace-nowrap w-full sm:w-max">
                <Img src={`/icons/${item.icons}`} alt={item.name} className="size-8 flex-shrink-0" />
