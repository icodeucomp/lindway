...
const dropdownVariants = {
  hidden: { opacity: 0, y: -20, scale: 0.95, transition: { duration: 0.2, ease: "easeInOut" } },
  visible: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.3, ease: "easeOut", staggerChildren: 0.08 } },
...
export const Header = ({ isDark }: { isDark?: boolean }) => {
  ...
  const [ref, openDropdown, toggleDropdown] = useToggleState();
  return (
    <>
      {/* Logo Header */}
                ...
                <menu className="flex flex-col list-none">
                  {navFeatureLists.map((item, index) => (
                    <motion.li key={index} variants={menuItemVariants} className="block sm:hidden">
                      <Link href={item.href} className="block px-4 py-3 text-sm transition-colors hover:bg-opacity-10 hover:bg-light" onClick={toggleDropdown}>
                        {item.name}
                      </Link>
                    </motion.li>
                  ))}
