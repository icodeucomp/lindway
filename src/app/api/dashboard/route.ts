...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const totalAllStockMyLindway: totalAllStock = await prisma.$queryRawUnsafe(
      `SELECT CAST(SUM((s ->> 'quantity')::int) AS INT) AS total FROM "products", unnest(sizes) AS s WHERE category = 'my lindway' ${queryYearAndMonth};`
    );
    const totalAllStockSimplyLindway: totalAllStock = await prisma.$queryRawUnsafe(
      `SELECT CAST(SUM((s ->> 'quantity')::int) AS INT) AS total FROM "products", unnest(sizes) AS s WHERE category = 'simply lindway' ${queryYearAndMonth};`
    );
    const totalAllStockLureByLindway: totalAllStock = await prisma.$queryRawUnsafe(
      `SELECT CAST(SUM((s ->> 'quantity')::int) AS INT) AS total FROM "products", unnest(sizes) AS s WHERE category = 'lure by lindway' ${queryYearAndMonth};`
    );
